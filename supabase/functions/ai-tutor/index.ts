...
Deno.serve(async(req: Request)=>{
 ...
 try{
  ...
  if(body.task==="create-assignment")system="Create a classroom English multiple-choice assignment with exactly 20 questions worth 5 marks each, totalling 100 marks. Every question must use this plain-text format: 1. Question text, then four separate lines A. option, B. option, C. option, D. option. Include only one correct option per question. For Paper 2 short-story assignments, include comprehension, setting, character, conflict, theme, vocabulary in context, inference and literary devices. For Paper 2 drama assignments, include plot, characters, dialogue, stage directions, conflict, theme, tone and dramatic techniques. When both are requested, balance the questions equally. After all questions add a section named MARKING GUIDE with each question number and correct letter. Match the requested level. Do not use markdown tables.";
  ...
  for(const candidate of candidates){
   try{
    ...
    const maxTokens=body.task==="create-assignment"?4000:body.task==="create-lesson"?2500:1200;
    const upstream=await fetch(useOpenRouter?"https://openrouter.ai/api/v1/chat/completions":"https://api.apinex.bond/v1/chat/completions",{method:"POST",signal:AbortSignal.timeout(25000),headers:{Authorization:`Bearer ${useOpenRouter?openRouterKey:key}`,"Content-Type":"application/json",...(useOpenRouter?{"HTTP-Referer":"https://english.tivalsdeveloper.site","X-Title":"English Study Co.Master"}:{})},body:JSON.stringify({model:candidate,messages:[{role:"system",content:system},...messages],temperature:.4,max_tokens:maxTokens,...(body.task==="dictionary"?{response_format:{type:"json_object"}}:{})})});
